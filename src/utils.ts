...
function getPoolAddress(
    ...
    let salt = ethers.utils.keccak256(
        ethers.utils.defaultAbiCoder.encode(["address", "address", "uint24"],
            [token0, token1, fee]))
...
export async function isUniswapV3Pool(poolAddress: string, poolAbi: string[],
    ...
    let uniswapPoolAddress: string = getPoolAddress(tokenA, tokenB, fee,
        factoryAddress, POOL_INIT_HASH)
